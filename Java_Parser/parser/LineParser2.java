...
public class LineParser2 {
	...
	public LineParser2(HashSet<String> l){
		this.transferList = l;
	}
	...
	// 2호선 순환선 (시청역 ~ 충정로역) -> 모든 지하철역을 하나로의 선으로 전부 연결
	public void setPeripheral() {
		for (int i = 0; i < listOfStation.size(); i++) {
			...
			if (Integer.toString(nowStation.getStationCode()).length() == 3) {
				if (i == 0) { // 시청역
					nowStation.getMovableStations().add(new MovableStation(listOfStation.get(42).getStationName(), 3));    // 충정로역
					nowStation.getMovableStations().add(new MovableStation(listOfStation.get(i + 1).getStationName(), 3)); // 을지로입구역
					...
					if (i != 42) { // 충정로역이 아닌 경우
						...
						nowStation.getMovableStations().add(new MovableStation(listOfStation.get(i - 1).getStationName(), 3)); // 아현역
						nowStation.getMovableStations().add(new MovableStation(listOfStation.get(0).getStationName(), 3));     // 시청역
					}
					...
					for (k = index; k < listOfStation.size(); k++) {
						...
						if (Integer.toString(nowStation.getStationCode()).substring(0, 3).equals(Integer.toString(listOfStation.get(k).getStationCode()).substring(0, 3))) {
							if (flag == true) {
								...
								if ((Integer.toString(listOfStation.get(k).getStationCode()).length() == 4 && Integer.toString(nowStation.getStationCode()).substring(0, 3).equals(Integer.toString(listOfStation.get(k).getStationCode()).substring(0, 3)))) {
	...
	public void setTransfer(SubwayMap subwaymap) {
		for (int i = 0; i < listOfStation.size(); i++) {
			...
			if (nowStation.getLineNum() == 'T') {
				nowStation.addLineData('2'); // 호선 정보 등록
				...
				if (i == 0) {
					if (subwaymap.get(nowStation.getStationName()) != null) { // 해당역이 기존 subwaymap에 존재 O
						...
						tr.addLineData('2'); // 호선 정보 등록
						
						if (tr.getStationName().equals("성수")) { // 성수역, 같은 호선에서 갈림길 有
							tr.setLineNum('P');
						...
						if (nowStation.getStationName().equals("성수")) { // 성수역, 같은 호선에서 갈림길 有
